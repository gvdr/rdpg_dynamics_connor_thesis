...
"""
Alg4 Synthetic Dataset: Polynomial Dynamics in P

Generates:
- True latent trajectory X(t) from Ẋ = (sum_k α_k P^k) X, P = X X'
...
Outputs:
- data/alg4/polynomial/polynomial_data.jls
"""
...
# Polynomial coefficients (degree K = length(alpha) - 1)
# Case: K=2
const ALPHA = [-0.0001, 0.3, -0.02]

# Initial cluster (single cloud)
const CENTERS = [
    [0.65, 0.35],
    [0.35, 0.65],
]
...
function dynamics!(du, u, p, t)
    return polynomial_dynamics!(du, u, p, t; n=N, d=D, alpha=ALPHA)
end
...
out_dir = joinpath("data", "alg4", "polynomial")
mkpath(out_dir)
out_path = joinpath(out_dir, "polynomial_data.jls")

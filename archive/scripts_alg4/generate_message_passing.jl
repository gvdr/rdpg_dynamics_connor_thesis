...
"""
Alg4 Synthetic Dataset: Message-Passing Dynamics

Generates:
- True latent trajectory X(t) from message-passing dynamics
...
Outputs:
- data/alg4/message_passing/message_passing_data.jls
"""
...
# g(x_i, x_j) = beta1 (x_j - x_i) + beta2 x_j
const BETA = [0.04, 0.01]

# Initial clusters (three communities)
const CENTERS = [
    [0.6, 0.0],
    [-0.3, 0.5],
    [-0.3, -0.5],
]
const NOISE_STD = 0.04
...
function dynamics!(du, u, p, t)
    return message_passing_dynamics!(du, u, p, t; n=N, d=D, beta=BETA)
end
...
out_dir = joinpath("data", "alg4", "message_passing")
mkpath(out_dir)
out_path = joinpath(out_dir, "message_passing_data.jls")

"""
    Pipeline utilities for RDPG Dynamics Examples
...
# SciML stack (others already loaded in training.jl)
...
export F, DATA_DIR, RESULTS_DIR, FILES
export data_path, results_path, save_data, load_data, data_exists
export rdpg_estimate, print_rdpg_diagnostics
export pairwise_distances, upper_tri, evaluate_positions, evaluate_series, print_evaluation
export position_loss, train_dynamics, recover_trajectory
export build_nn, build_correction_nn
export plot_trajectory_evolution, plot_trajectory_comparison, plot_metrics, plot_learning_curves
export phase_generate, phase_estimate, phase_train, phase_evaluate, phase_visualize
export check_phases, print_status, print_summary_table
...
function phase_generate(example_name::String, dynamics_fn!, X0::Matrix{Float64};
    ...
    # Use Tsit5 with high maxiters for long simulations
    sol = solve(prob, Tsit5(), saveat=tsteps,
                abstol=1e-6, reltol=1e-6, maxiters=10_000_000)

    # Check solution status
    if sol.retcode != :Success && sol.retcode != ReturnCode.Success
        @warn "ODE solver returned: " * string(sol.retcode) * ". Using available timesteps."
    end

    # Use actual solution length, not expected
    actual_steps = length(sol.u)
    X_true_series = [reshape(sol.u[t], n, d) for t in 1:actual_steps]

    println("  Generated " * string(actual_steps) * " timesteps (requested " * string(T_steps) * ")")
